...
function addLanguage(lang) {
  ...
  for (let i = 0; i < mainDiv.children.length; i++) {
    if (mainDiv.children[i].children.length > 0) {
      ...
      id.includes("Date=0000")
        ? (id = id.split("Date=000")[0] + "Date=0000")
        : (id = id.split("Date=")[0]);
...
function appendRepeatable(elID) {
  ...
  for (let i = 1; i < 1000; i++) {
    ...
    newDiv.setAttribute(
      "id",
      repeat.getAttribute("id") + String(i) + "Date=" + copticReadingsDate
    );
    ...
    for (let x = 0; x < newDiv.children.length; x++) {
      ...
      child.setAttribute(
        "id",
        newDiv.getAttribute("id") +
          child.getAttribute("id").slice(13, child.getAttribute("id").length)
      );
...
function showPrayers(prayers, prayersArray, languages) {
  ...
  allLanguages.map(
    (l) => (document.getElementById(mainDiv.id + l).innerHTML = "")
  );
  ...
  prayers.map((prayer) =>
    retrievePrayersFromAnArray(prayersArray, prayer, languages)
  );
  closeSideBar();
}
      ...
      if (firstElement == idsArray[0] || firstElement == idsArray[1]) {
        for (let x = 0; x < prayersArray[i].length; x++) {
          ...
          if (document.getElementById(mainDiv.id + lang)) {
            ...
            console.log(
              "There is no document element in the DOM to show this language: ",
              lang
            );
          }
        }
...
function getTextAndAppendElementToMainDiv(elID) {
  ...
  if (elID.startsWith("creatHtml")) {
    ...
    if (typeof prayer === "undefined") {
      console.log(
        "no html element with the provided ID:  " + elID + " returned undefined"
      );
      return;
...
function setCopticReadingsDate(coptDate) {
  ...
  if (greatLentOrPentecostal != "") {
    ...
    if (greatLentOrPentecostal.includes("GreatLent")) {
      if (
        btnDayReadings.children.indexOf(btnReadingsPropheciesDawn) == -1 &&
        todayDate.getDay() != 0 &&
        todayDate.getDay() != 6
      ) {
        ...
        btnDayReadings.children.splice(
          btnDayReadings.children.indexOf(btnReadingsPropheciesDawn),
          1
        );
      }
      if (
        btnDayReadings.children.indexOf(btnReadingsGospelNight) == -1 &&
        todayDate.getDay() == 0
      ) {
